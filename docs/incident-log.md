...
**Incident 9 (2026-08-22):** ClickRank AI Overview Tracker submissions for Post #5 appeared to fail: the modal returned a generic error, the URL lookup logged `Unexpected token '<'` because an HTML response was parsed as JSON, and a later attempt remained on `Processing...`. The authenticated dashboard was healthy, but the tracker write was asynchronous; after leaving the existing Chrome tab and reopening the tracker, the row appeared (8/8 tracked keywords) about an hour later.
- **Root cause:** ClickRank's URL-search/submission path is asynchronous and can return a transient HTML/error response while processing. The UI error and processing state were not authoritative proof of failure.
- **Rule:** After any ClickRank submission, wait for processing, reload the same authenticated tab, and verify both the tracked-keyword count and the exact keyword/URL/market/language row before retrying. Never create a duplicate based only on a toast, JSON parse error, or temporary `Processing...` state. `Not Found`, `N/A`, and `0%` are valid tracked-result values.
